...
const EI = (function () {
  ...
  function scoreGame(g, opp, ownNatRank) {
    ...
    if (win) {
      ...
      if (r <= 30 && gap >= 25 && am <= 10) { out.qualityLoss = am <= 3 ? 16 : am <= 7 ? 10 : 5; out.base += out.qualityLoss; }
      /* bad-loss detection */
  ...
  function compute(throughWeek) {
    ...
    for (let pass = 0; pass < 4; pass++) {
      result = TEAMS.map(t => {
        ...
        let score = 0, rows = [], qualityWins = 0, qualityLosses = 0, badLosses = 0, winStrength = 0, pd = 0;
  ...
  function oppLabel(g) { return g.fcs ? g.oppName + ' (FCS)' : (g.oppNatRank <= 25 ? 'No. ' + g.oppNatRank + ' ' + g.oppName : g.oppName); }
  ...
  function reason(t) {
    ...
    if (g.label === 'FCS loss') return dir + ' after losing ' + scoreLine(g) + ' to ' + g.oppName + ', an FCS opponent — the largest single penalty in the Elevation Index.';
