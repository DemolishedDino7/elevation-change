...
3. Upload every file in this folder, all in one folder, no subfolders.
...
Open `season-opener.html`. Everything you need to change is in the `<article>` section:
...
1. Copy `season-opener.html` to a new file, e.g. `articles/boise-week-two.html`.
...
- one HTML file per post, e.g. `season-opener.html`
- `styles.css` — all styling; change colors at the top under `:root`
- `logo.png`, `logo-mark.png`, `logo-transparent.png` — logo files
